...
if [ -d ./libharfbuzz/cmake/${ANDROID_ABI} ]; then

cd ./libharfbuzz/cmake/${ANDROID_ABI}

else

mkdir -p ./libharfbuzz/cmake/${ANDROID_ABI}
cd ./libharfbuzz/cmake/${ANDROID_ABI}
...
${CMAKE} ${DALI_DIR}/android-dependencies/libharfbuzz -DCMAKE_C_COMPILER=${CMAKE_C_COMPILER} -DCMAKE_CXX_COMPILER=${CMAKE_CXX_COMPILER} -DCMAKE_INSTALL_PREFIX=${CMAKE_INSTALL_PREFIX} -DCMAKE_TOOLCHAIN_FILE=${CMAKE_TOOLCHAIN_FILE} -DANDROID_PLATFORM=${ANDROID_PLATFORM} -DANDROID_ABI=${DANDROID_ABI} -DANDROID_NDK=${ANDROID_NDK} -DANDROID_STL=c++_shared -DCMAKE_CXX_FLAGS='-fexceptions -frtti -w -Wall -std=gnu++11 -I${CMAKE_INSTALL_INCLUDEDIR}/freetype2/freetype -I${CMAKE_INSTALL_INCLUDEDIR}/freetype2/freetype/config -L${CMAKE_INSTALL_LIBDIR}' -DCMAKE_INSTALL_INCLUDEDIR=${CMAKE_INSTALL_INCLUDEDIR} -DCMAKE_INSTALL_LIBDIR=${CMAKE_INSTALL_LIBDIR} -DCMAKE_BUILD_TYPE=$@ -DHB_BUILTIN_UCDN=ON -DHB_HAVE_FREETYPE=ON -DBUILD_SHARED_LIBS=ON
